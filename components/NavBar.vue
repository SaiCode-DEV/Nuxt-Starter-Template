<template>
  <v-navigation-drawer v-model="drawer" app temporary>
    <v-list dense nav>
      ...
      <v-list-item v-if="status === 'authenticated'" to="/dashboard">
        <template #prepend>
  ...
  <v-app-bar app flat class="frosted-navbar" scroll-behavior="elevate">
    ...
    <Icon name="mdi:rocket-launch" size="32" class="mr-3" />    <v-toolbar-title class="font-weight-bold">
      SaiCode's Template
    </v-toolbar-title>
      ...
      <v-menu>
        <template #activator="{ props }">
          <v-btn v-bind="props" variant="text">
            <Icon name="mdi:account-circle" class="mr-2" />
            {{ data?.user?.name || "User" }}
